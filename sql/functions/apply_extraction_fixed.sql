-- Fixed apply_extraction with proper normalization matching the hash
...
BEGIN
  ...
  -- 2) Upsert obligations with proper normalization
  FOR ob IN SELECT value FROM jsonb_array_elements(COALESCE(p_payload->'obligations','[]'::jsonb)) LOOP
    ...
    -- Normalize to match hash generation exactly
    trig  := CASE WHEN trig  IS NULL THEN NULL ELSE regexp_replace(trim(lower(trig)),  '\s+', ' ', 'g') END;
    party := CASE WHEN party IS NULL THEN NULL ELSE regexp_replace(trim(lower(party)), '\s+', ' ', 'g') END;
    descr := CASE WHEN descr IS NULL THEN NULL ELSE regexp_replace(trim(lower(descr)), '\s+', ' ', 'g') END;
    ...
    -- Race-safe upsert using the hash-based unique index
  ...
  -- 4) Mark stale obligations as superseded
